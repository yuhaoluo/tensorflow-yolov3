...
def test_conv(input_data, filters_shape, trainable, name, downsample=False, activate=True, bn=False):
    ...
    with tf.variable_scope(name):
        ...
        with tf.name_scope('FRN'):
            tau = tf.Variable(1,dtype=tf.float32)
            beta = tf.Variable(1,dtype=tf.float32)
            gamma = tf.Variable(0,dtype=tf.float32)

        conv = FRNLayer(conv,tau,beta,gamma)
