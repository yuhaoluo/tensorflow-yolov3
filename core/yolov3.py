...
class YOLOV3(object):
    ...
    def __build_nework(self, input_data):
        ...
        ## conv_lbbox (batch,13,13,3*(4+1+num_class))
        ## conv_mbbox (batch,26,26,3*(4+1+num_class))
        ## conv_sbbox (batch,52,52,3*(4+1+num_class))

    def decode(self, conv_output, anchors, stride):
        """
        将原始的卷积输出解码成:
             bounding box坐标（在原始图片尺寸） 4
             obj conf值（经过sigmoid转换）     1
             类别prob（经过sigmod转换)     num_class
        args:
            anchors: (3,2),current scale anchors
        return 
    ...
    def bbox_iou(self, boxes1, boxes2):
        ...
        ## convert form (x,y,w,h) to (xmin,ymin,xmax,ymax)
        boxes1 = tf.concat([boxes1[..., :2] - boxes1[..., 2:] * 0.5,
                            boxes1[..., :2] + boxes1[..., 2:] * 0.5], axis=-1)
        boxes2 = tf.concat([boxes2[..., :2] - boxes2[..., 2:] * 0.5,
                            boxes2[..., :2] + boxes2[..., 2:] * 0.5], axis=-1)
    ...
    def compute_loss(self, label_sbbox, label_mbbox, label_lbbox, true_sbbox, true_mbbox, true_lbbox):
        '''
        args:
            label_box (batch,size,size,3,5+n_class)
            true_box  (batch,n_box,4)
        '''
